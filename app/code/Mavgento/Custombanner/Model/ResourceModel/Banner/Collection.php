...
namespace Mavgento\Custombanner\Model\ResourceModel\Banner;
...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    {
        $this->_init('Mavgento\Custombanner\Model\Banner', 'Mavgento\Custombanner\Model\ResourceModel\Banner');
    }
    ...
    public function __construct(
        ...
        \Mavgento\Custombanner\Model\Slider $slider,
        $connection = null,
    ...
    public function joinSliderTitle()
    {
        $this->getSelect()->joinLeft(
            ['sliderTable' => $this->getTable('mavgento_custombanner_slider')],
    ...
    public function addFieldToFilter($field, $condition = null)
    {
        ...
        if (in_array($field, $attributes) && $storeViewId) {
            if (!in_array($field, $this->_addedTable)) {
                ...
                $this->getSelect()
                    ->joinLeft(array($field => $this->getTable('mavgento_custombanner_value')), $sql, array());
    ...
    public function getBannerCollection($sliderId)
    {
        ...
        /** @var \Mavgento\Custombanner\Model\ResourceModel\Banner\Collection $bannerCollection */
        $bannerCollection = $this->setStoreViewId($storeViewId)
            ->addFieldToFilter('slider_id', $sliderId)
            ->addFieldToFilter('status', \Mavgento\Custombanner\Model\Status::STATUS_ENABLED)
            ->addFieldToFilter('start_time', ['lteq' => $dateTimeNow])
            ->addFieldToFilter('end_time', ['gteq' => $dateTimeNow])
            ->setOrder('order_banner', 'ASC');

        if ($this->_slider->getSortType() == \Mavgento\Custombanner\Model\Slider::SORT_TYPE_RANDOM) {
